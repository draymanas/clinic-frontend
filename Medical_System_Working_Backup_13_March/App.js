...
<marquee behavior="scroll" direction="right" scrollamount="5" style={{ fontSize: '32px', color: '#d32f2f', fontWeight: '900' }}>
...
<button style={{ 
  ...
  alignItems: 'center'
              ...
              <div style={{ position: 'absolute', top: '15px', left: '15px', background: '#e8f5e9', color: '#2e7d32', padding: '5px 12px', borderRadius: '12px', fontSize: '14px', fontWeight: 'bold' }}>
                 {doc.fee} ج.م
              </div>
            ...
            {showModal && (
                <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                    <div style={{ backgroundColor: '#fff', padding: '25px', borderRadius: '15px', width: '350px' }}>
                        ...
                        <input placeholder="اسمك" onChange={e => setPatientData({...patientData, name: e.target.value})} style={{...inputStyle, marginTop:'10px'}} />
                        <input placeholder="موبايلك" onChange={e => setPatientData({...patientData, mobile: e.target.value})} style={{...inputStyle, marginTop:'10px'}} />
    ...
    <button onClick={() => setCurrentUser({role: 'admin_check'})} style={{flex:1, padding: '10px', cursor:'pointer', borderRadius: '8px', border: currentUser?.role === 'admin_check' ? '2px solid #3498db' : '1px solid #ddd'}}>أنا أدمن</button>
                ...
                {currentUser?.role === 'patient' && (
                    <div style={{ display: 'grid', gap: '10px' }}>
                        <input placeholder="الاسم" style={inputStyle} onChange={(e) => setCurrentUser({...currentUser, name: e.target.value})} />
                        <input placeholder="الموبايل" style={inputStyle} onChange={(e) => setCurrentUser({...currentUser, mobile: e.target.value})} />
                        <button onClick={() => { setShowLoginModal(false); }} style={{ background: '#27ae60', color: '#fff', border: 'none', padding: '12px', borderRadius: '8px', cursor: 'pointer' }}>دخول وحجز</button>
                    </div>
                )}

                {currentUser?.role === 'admin_check' && (
                    <div style={{ display: 'grid', gap: '10px' }}>
                        <input type="password" placeholder="كلمة سر الأدمن" style={inputStyle} onKeyDown={(e) => {
...
{currentUser?.role === 'doctor_check' && (
    <div style={{ display: 'grid', gap: '10px' }}>
        <input placeholder="أدخل رقم الموبايل المسجل به" style={inputStyle} 
          onKeyDown={async (e) => {
            if(e.key === 'Enter') {
                ...
                if(doc) {
                    ...
                    setActivePage('doctor_dashboard'); // نفتح لوحته فوراً
                    setShowLoginModal(false);
                } else {
